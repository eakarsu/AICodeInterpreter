...
// Feature: Agentic data-analyst that plans, executes, and reports analyses end-to-end
import React, { useState } from 'react';
import api from '../services/api';

export default function AgenticDataAnalystThatPlansExecutesAndReporPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-agentic-data-analyst-that-plans-executes-and-repor/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Agentic data-analyst that plans, executes, and reports analyses end-to-end</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-agentic-data-analyst-that-plans-executes-and-repor/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
